...
class BreathingController {
    static async getPatterns(req, res) {
        try {
            await breathing_service_1.BreathingService.initializeDefaultPatterns();
            const pattern = await breathing_service_1.BreathingService.getPattern(req.params.name);
            if (!pattern) {
    ...
    static async startSession(req, res) {
        ...
        try {
            ...
            const pattern = await breathing_service_1.BreathingService.getPattern(patternName);
            if (!pattern) {
                res.status(400).json({ error: 'Invalid pattern' });
                return;
            }
            const session = await breathing_service_1.BreathingService.startSession(userId, patternName, stressLevelBefore);
            res.json(session);
        }
        catch (error) {
    ...
    static async completeSession(req, res) {
        ...
        try {
            ...
            const session = await breathing_service_1.BreathingService.getUserSessionById(sessionId);
            if (!session) {
            ...
            const updatedSession = await breathing_service_1.BreathingService.completeSession(sessionId, completedCycles, stressLevelAfter);
    ...
    static async getUserSessions(req, res) {
        ...
        try {
            ...
            const sessions = await breathing_service_1.BreathingService.getUserSessions(userId, limitParam);
            res.json(sessions);
        }
        catch (error) {
    ...
    static async getEffectiveness(req, res) {
        ...
        try {
            ...
            const effectiveness = await breathing_service_1.BreathingService.getEffectiveness(userId);
