...
class RecommendationService {
     ...
     */
    static async getPersonalizedRecommendations(userId, limit = 3) {
        try {
            // Get user's recent sessions
            const recentSessions = await meditation_session_model_1.MeditationSession.find({ userId })
                .sort({ completedAt: -1 })
                .limit(10)
                .lean();
            // Get user's stress assessments
            const recentStressAssessments = await stress_model_1.StressAssessmentLegacy.find({ userId })
                .sort({ timestamp: -1 })
                .limit(5)
                .lean();
            ...
            const recommendations = await this.generateRecommendations(userId, recentSessions, recentStressAssessments, user, analytics);
     ...
     */
    static async generateRecommendations(userId, recentSessions, recentStressAssessments, user, analytics) {
     ...
     */
    static getTriggerBasedRecommendations(triggers) {
        const recommendations = [];
        // Map common triggers to specific meditation techniques
        const triggerRecommendationMap = {
            'work': {
                type: 'meditation',
     ...
     */
    static async getUserPreferenceRecommendations(preferences) {
        const recommendations = [];
        if (!preferences)
            return recommendations;
     ...
     */
    static isCurrentTimePreferred(currentHour, preferredTimes) {
     ...
     */
    static getTechniqueTitle(technique) {
        const titleMap = {
     ...
     */
    static mapTechniqueToType(technique) {
        const typeMap = {
            '4-7-8': 'breathing',
