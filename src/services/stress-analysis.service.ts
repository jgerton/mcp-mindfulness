...
export class StressAnalysisService {
   ...
   */
  static async analyzeStressData(
   ...
   */
  private static calculateAverageStressLevel(assessments: any[]): number {
   ...
   */
  private static analyzeStressTrend(assessments: any[]): string {
   ...
   */
  private static identifyCommonTriggers(assessments: any[], limit: number = 5): any[] {
   ...
   */
  private static identifyCommonSymptoms(assessments: any[], limit: number = 5): any[] {
    const symptomCounts: Record<string, number> = {};
    
    // Count occurrences of each symptom
    assessments.forEach(assessment => {
      if (Array.isArray(assessment.symptoms)) {
        assessment.symptoms.forEach((symptom: string) => {
   ...
   */
  private static identifyPeakStressTimes(assessments: any[], limit: number = 3): any[] {
    const hourlyStress: Record<number, { total: number, count: number }> = {};
    ...
    return hourlyAverages
      ...
      .map(item => ({
        ...item,
        timeOfDay: this.formatHourToTimeOfDay(item.hour)
      }));
  }
   ...
   */
  private static formatHourToTimeOfDay(hour: number): string {
   ...
   */
  private static calculateAverageStressLevelByDayType(assessments: any[], isWeekday: boolean): number {
   ...
   */
  static async identifyStressTriggers(
    userId: string | mongoose.Types.ObjectId,
    limit: number = 5
  ): Promise<any[]> {
    try {
      // Get all stress assessments for the user
      const assessments = await StressAssessmentLegacy.find({ userId }).lean();
      ...
      assessments.forEach(assessment => {
        if (Array.isArray(assessment.triggers)) {
          assessment.triggers.forEach((trigger: string) => allTriggers.add(trigger));
        }
      });
      
      // Calculate correlation between each trigger and stress level
      const triggerCorrelations: Record<string, { sum: number, count: number }> = {};
