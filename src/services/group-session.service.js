...
class GroupSessionService {
    static async createSession(hostId, meditationId, title, scheduledTime, duration, options = {}) {
        var _a;
        const now = new Date();
        if (scheduledTime < now) {
            throw new Error('Cannot schedule session in the past');
        }
        const session = await group_session_model_1.GroupSession.create({
    ...
    static async startSession(sessionId, hostId) {
        const session = await group_session_model_1.GroupSession.findOne({
            _id: sessionId,
            hostId,
            status: 'scheduled'
        });
    ...
    static async completeSession(sessionId, userId, sessionData) {
        const session = await group_session_model_1.GroupSession.findById(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
        ...
        if (session.participants.length >= 3) {
            await achievement_service_1.AchievementService.processGroupSession(userId);
        }
        return session;
    }
    static async leaveSession(sessionId, userId) {
        ...
        await chat_service_1.ChatService.addMessage(sessionId, userId, `${user.username} has left the session`, 'system');
        return session;
    }
    static async cancelSession(sessionId, hostId) {
        const session = await group_session_model_1.GroupSession.findOne({
            _id: sessionId,
            hostId,
        ...
        await chat_service_1.ChatService.addMessage(sessionId, hostId, 'Session has been cancelled by the host', 'system');
        return session;
    }
    static async getUpcomingSessions(userId) {
        ...
        const friendIds = await friend_service_1.FriendService.getFriendList(userId)
    ...
    static async getUserSessions(userId) {
        const sessions = await group_session_model_1.GroupSession.find({
            $or: [
    ...
    static async endSession(sessionId, hostId) {
        ...
        await chat_service_1.ChatService.addSystemMessage(sessionId, 'Session ended by the host');
        return session;
    }
}
