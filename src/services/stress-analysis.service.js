...
class StressAnalysisService {
     ...
     */
    static async analyzeStressData(userId, startDate, endDate) {
        try {
            // Set default date range if not provided (last 30 days)
            const end = endDate || new Date();
            const start = startDate || new Date(end.getTime() - 30 * 24 * 60 * 60 * 1000);
            // Get stress assessments for the specified period
            const assessments = await stress_model_1.StressAssessmentLegacy.find({
                userId,
     ...
     */
    static async identifyStressTriggers(userId, limit = 5) {
        try {
            // Get all stress assessments for the user
            const assessments = await stress_model_1.StressAssessmentLegacy.find({ userId }).lean();
