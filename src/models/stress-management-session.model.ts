...
export enum StressManagementTechnique {
  ...
  ProgressiveMuscleRelaxation = 'progressive_muscle_relaxation',
  GuidedImagery = 'guided_imagery',
...
/**
 * Interface for stress management session feedback
 */
export interface IStressManagementFeedback {
  effectivenessRating: number; // 1-5 scale
  stressReductionRating: number; // 1-5 scale
  comments: string;
  improvements: string[];
}

/**
 * Interface representing a StressManagementSession document in MongoDB
 */
export interface IStressManagementSession extends IBaseWellnessSession {
  technique: StressManagementTechnique;
  stressLevelBefore: number; // 1-10 scale
  stressLevelAfter?: number; // 1-10 scale
  guidedSessionId?: mongoose.Types.ObjectId;
  triggers?: string[];
  physicalSymptoms?: string[];
  emotionalSymptoms?: string[];
  effectiveness?: number; // 1-5 scale
  feedback?: IStressManagementFeedback;
  
  // Virtual properties
  stressReduction: number;
  
  // Additional methods
  addFeedback(feedback: IStressManagementFeedback): Promise<IStressManagementSession>;
}

// Create a base model from the base schema
const BaseWellnessSession = mongoose.model<IBaseWellnessSession>('BaseWellnessSession', baseWellnessSessionSchema);

// Create the StressManagementSession schema as a discriminator of the base schema
const StressManagementSessionSchema = new Schema({
  technique: {
    type: String,
  ...
  feedback: {
    ...
    improvements: {
      ...
      validate: {
        validator: function(improvements: string[]) {
...
StressManagementSessionSchema.virtual('stressReduction').get(function(this: IStressManagementSession) {
...
StressManagementSessionSchema.methods.addFeedback = async function(
  this: IStressManagementSession,
  feedback: IStressManagementFeedback
): Promise<IStressManagementSession> {
  if (this.status !== WellnessSessionStatus.Completed) {
...
StressManagementSessionSchema.methods.processAchievements = async function(this: IStressManagementSession) {
  try {
...
// Pre-save middleware
StressManagementSessionSchema.pre('save', function(this: IStressManagementSession, next) {
  // If completing the session without setting stressLevelAfter, copy the before value
  if (this.status === WellnessSessionStatus.Completed && !this.stressLevelAfter) {
...
export const StressManagementSession = BaseWellnessSession.discriminator<IStressManagementSession>(
  'StressManagementSession', 
  StressManagementSessionSchema
);

export default StressManagementSession; 